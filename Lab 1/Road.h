...
public:
  ...
  const float DEFAULT_WIDTH_FT = 10, DEFAULT_LENGTH_M = 1, DEFAULT_THICKNESS_IN = 5;
  ...
  float asphalt ( float thickness)
  {
    if (thickness > 0)
      {
      //Convert length and thicnkess to feet, and multiply
      return  (m_Length_m * FEET_PER_MILE) * m_Width_ft * (thickness / 12.0 );
      }
    else
      return  (m_Length_m * FEET_PER_MILE) * m_Width_ft * (5.0 / 12.0 );
  }
